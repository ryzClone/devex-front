...
export default function History({ closeModal, user }) {
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      const response = await fetch(`${BACK_API}api/history?${queryParams}`, {
        method: "GET",
        headers: { Authorization: `Bearer ${token}` },
      });

      // Handle 403 status for token expiration
            ...
            <tbody>
              {historyData.map((row, index) => (
                <tr key={row.id}>
                  ...
                  <td>{row.equipment.inventory_number}</td>
                  <td>{row.description}</td>
                  <td>{row.equipment.status}</td>
                   <td>{row.action}</td>
                  <td>
