...
import AddTExnika from "./addtexnika";
...
export default function Texnika() {
  ...
  const [addTexnika, setAddtexnika] = useState("");
  ...
  const readAcception = async () => {
    ...
    try {
      ...
      console.log("Error during readAcception:", error);
    }
  };
  ...
  const handleSendToUnused = async (equipment) => {
    try {
      const token = localStorage.getItem("token");
      ...
      const response = await fetch(`${BACK_API}api/movetounused`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
          ...
          <div className="menu-options">
            ...
            {activeMenuId === user.id && (
              <div className="dropdown-menu">
                ...
                <button
                  onClick={() => {
                    handleSendToUnused(user);
                    setActiveMenuId(null); // Menyuni yopish
                  }}
                >
                  Направить в списании
                </button>
                <button
                  onClick={() => {
                    handleUpdate(user);
  ...
  const handleAddtexnika = async (e) => {
    const data = {
      name: e.naimenovaniya_tex || "",
      inventory_number: e.inv_tex || "",
      serial_number: e.seriyniy_nomer || "",
    ...
    try {
      const response = await fetch(`${BACK_API}api/addtexnika`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        const result = await response.json();
        setSuccess(true);
        setText(result.message || "Texnika muvaffaqiyatli qo'shildi!");
      } else {
        const errorResult = await response.json();
        setSuccess(false);
        setText(errorResult.message || "Texnika qo'shishda xatolik yuz berdi");
      }
    } catch (error) {
      setSuccess(false);
      setText(error.message || "Texnika qo'shishda xatolik yuz berdi");
    } finally {
      setShowSuccess(true);
      setAddtexnika(false);
      setTimeout(() => {
        ...
        <div className="controls">
          <div className="dropdown">
            ...
            <div className="dropdown-menu">
              <a
                href="#!"
                onClick={() => handleSortStatusClick("naimenovaniya_tex")}
              >
                Наименования тех
              </a>
              ...
              <a
                href="#!"
                onClick={() => handleSortStatusClick("seriyniy_nomer")}
              >
                Серийный номер
              </a>
          ...
          <button
            ...
            onClick={() => setAddtexnika(true)}
          >
            <FaUserPlus /> Добавить техника
          </button>
        </div>
      </div>
