...
export default function TexnikaRepair() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (response.status === 403) {
          ...
          // Trigger onClose after showing the error message
          setTimeout(() => {
  ...
  const handleSendToTexnika = async (equipment) => {
    try {
      ...
      const userId = localStorage.getItem("user_id"); // Get the username
    ...
    if (!userId) {
      ...
      // Send request to the 'texnika_unused' endpoint
      ...
      // Check if the response status is 403 (Token expired)
      if (response.status === 403) {
        setText("Срок действия вашего токена истек");
        setSuccess(false);
        setShowSuccess(true);
      ...
      console.error("Error while sending to texnika:", error);
      setSuccess(false);
      setText(error.message || "Texnika o'zgartirishda xatolik yuz berdi");
    } finally {
      // Show success/error modal or message
      setShowSuccess(true);
      setTimeout(() => {
        setShowSuccess(false);
      ...
      <div className="users-header">
        ...
        <div className="controls">
          <div className="dropdown">
            ...
            <div className="dropdown-menu">
              <a href="#!" onClick={() => handleSortStatusClick("equipment_name")}>
                Наименования тех
              </a>
              <a href="#!" onClick={() => handleSortStatusClick("inventory_number")}>
                ИНВ номер
              </a>
              <a href="#!" onClick={() => handleSortStatusClick("serial_number")}>
                Серийный номер
              </a>
              <a href="#!" onClick={() => handleSortStatusClick("mac")}>
                Mac address
              </a>
            </div>
          </div>
